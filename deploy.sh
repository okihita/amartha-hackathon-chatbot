...
gcloud run deploy ${SERVICE_NAME} \
  ...
  --timeout 300 \
  --concurrency 80 \
  --set-env-vars "GCP_PROJECT_ID=${PROJECT_ID},MY_VERIFY_TOKEN=${MY_VERIFY_TOKEN},WHATSAPP_TOKEN=${WHATSAPP_TOKEN},PHONE_NUMBER_ID=${PHONE_NUMBER_ID},GEMINI_API_KEY=${GEMINI_API_KEY},GCS_BUCKET=${GCS_BUCKET}" \
  --quiet
